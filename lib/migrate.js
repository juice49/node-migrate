...
function migrate (set, direction, options, fn) {
  // console.log('migrate', direction, options, fn)
  options = options || {}

  if (typeof options === 'string') {
    options = {
      migrationName: options
    }
  }
  ...
  } else if ((toIndex = positionOfMigration(set.migrations, options.migrationName)) === -1) {
    return fn(new Error('Could not find migration: ' + options.migrationName))
  }
  ...
  function next (migration) {
    ...
    var returnValue = options.args
      ? migration[direction](options.args, cb) // xxx
